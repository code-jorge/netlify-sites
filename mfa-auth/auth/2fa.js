...
exports.hotp = hotpGenerate = (options) => {
	...
	if (options.length != null)
		console.warn(
			"Deprecation Notice - Specifying token digits using `length` is no longer supported. Use `digits` instead."
		)
...
exports.hotp.verifyDelta = hotpVerifyDelta = (options) => {
	...
	if (token.length !== digits) {
		throw new Error("hotpVerifyDelta - Wrong token length")
	}
	...
	// fail if token is NA
	if (isNaN(token)) {
		throw new Error("hotpVerifyDelta - Token is not a number")
	}
...
exports._counter = _counter = (options) => {
	...
	if (options.initial_time != null)
		console.warn(
			"Deprecation Notice - Specifying the epoch using `initial_time` is no longer supported. Use `epoch` instead."
		)
...
exports.otpauthURL = otpauthURL = (options) => {
	...
	switch (type) {
		...
		case "hotp":
			break
		default:
			throw new Error(`Invalid type \`${type}\`; must be \`hotp\` or \`totp\``)
	}
	...
	if (algorithm != null) {
		switch (algorithm.toUpperCase()) {
			...
			case "SHA512":
				break
			default:
				console.warn("Warning - Algorithm generally should be SHA1, SHA256, or SHA512")
		}
		...
		if (isNaN(digits)) {
			...
			switch (parseInt(digits, 10)) {
				...
				case 8:
					break
				default:
					console.warn("Warning - Digits generally should be either 6 or 8")
			}
		}
		query.digits = digits
	}
